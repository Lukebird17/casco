...
[
  ...
  },
  {
    ...
    "字节编号": "3",
    "长度": "8字节",
    "信息位定义及说明": "用于将GAL包双网传输的唯一标识。发送方设置，双网发送数据时，同一GAL包的双网序列号必须保证相同。通信连接建立后，通信双方将各自的“双网序列号”置0，并随着GAL包的发送递增计数，每发一包，计数加1，到达8字节限制后又变成0。接收方接收双网数据时，如果当前GAL包的双网序列号等于或小于上一个接收的GAL包的双网序列号，则丢弃当前GAL包"
  },
  {
    "信息域定义": "双网序列号",
    "字节编号": "4",
...
[
  ...
  {
    "信息域定义": "*序列号有效值为1~(231-1)。发送方应保证生成两包信息包时,两包信息报中的“本方消息序列号”字段的差值与“通信周期”相乘等于生成两包消息的时间间隔,主要用于心跳信息包交互时判断通信延时的参考依据,但不强制要求通信延时的处理方式。当未收到对方消息时,此字段应填写0xFFFFFFF。ATS判断“协议版本号”字段校验不通过时,应进行丢包或断开通信处理。",
    "字节编号": "*序列号有效值为1~(231-1)。发送方应保证生成两包信息包时,两包信息报中的“本方消息序列号”字段的差值与“通信周期”相乘等于生成两包消息的时间间隔,主要用于心跳信息包交互时判断通信延时的参考依据,但不强制要求通信延时的处理方式。当未收到对方消息时,此字段应填写0xFFFFFFF。ATS判断“协议版本号”字段校验不通过时,应进行丢包或断开通信处理。",
    "字段": "*序列号有效值为1~(231-1)。发送方应保证生成两包信息包时,两包信息报中的“本方消息序列号”字段的差值与“通信周期”相乘等于生成两包消息的时间间隔,主要用于心跳信息包交互时判断通信延时的参考依据,但不强制要求通信延时的处理方式。当未收到对方消息时,此字段应填写0xFFFFFFF。ATS判断“协议版本号”字段校验不通过时,应进行丢包或断开通信处理。",
    "长度": "*序列号有效值为1~(231-1)。发送方应保证生成两包信息包时,两包信息报中的“本方消息序列号”字段的差值与“通信周期”相乘等于生成两包消息的时间间隔,主要用于心跳信息包交互时判断通信延时的参考依据,但不强制要求通信延时的处理方式。当未收到对方消息时,此字段应填写0xFFFFFFF。ATS判断“协议版本号”字段校验不通过时,应进行丢包或断开通信处理。",
    "信息位定义及说明": "*序列号有效值为1~(231-1)。发送方应保证生成两包信息包时,两包信息报中的“本方消息序列号”字段的差值与“通信周期”相乘等于生成两包消息的时间间隔,主要用于心跳信息包交互时判断通信延时的参考依据,但不强制要求通信延时的处理方式。当未收到对方消息时,此字段应填写0xFFFFFFF。ATS判断“协议版本号”字段校验不通过时,应进行丢包或断开通信处理。"
  }
]
...
[
  ...
  },
  {
    "信息域定义": "当前包号",
    "字节编号": "7",
    "报文内容": "正在发送的信息包序号，从1开始",
    "说明": "应用层信息包需要拆分时，此信息包在所有信息包中的序号"
  },
  {
    "信息域定义": "当前包号",
    "字节编号": "8",
...
[
  ...
  },
  {
    "字段": "时间",
    "长度字节": "9",
    "说明": "本信息包产生时间。以数字表示年(2字节)、月、日、时、分、秒、毫秒(2字节)"
  },
  {
    "字段": "车站信息个数",
    "长度字节": "1",
  ...
  },
  {
    "字段": "设备信息1",
    "长度字节": "设备类型",
    "说明": "1数值,取值见附录A.1。其他值无效"
  },
  {
    "字段": "设备信息1",
    "长度字节": "设备编号",
    "说明": "4每站每种设备类型下单独编号,设备类型和设备编号一起应能唯一标识一个站内的设备。设备编号应优先采用互联互通信号系统"
  }
]
...
[
  {
    "字    段": "设备编号",
    "长度/字节": "4",
    "说    明": "设备编号。当互联互通信号系统设备编号文件未规定该设备的编号时,可由信息发送方厂家自行配置"
  },
  {
    "字    段": "色码长度",
    "长度/字节": "1",
  ...
  },
  {
    "字    段": "色码",
    "长度/字节": "N",
    "说    明": "N个字节顺序组成(bit0~bit8N-1)这8N个bit位。色码意为设备状态组位码,假设设备有n种(n≤8N)独立状态,第i(0≤i&lt;8N)种状态占用Mi位,这Mi位叫做第i个组位,则色码的8N位由这些状态组位组成,设备状态的扩展即为扩组位。色码定义见附录A.2"
  },
  {
    "字    段": "设备信息2"
  },
...
[
  ...
  },
  {
    "字 段": "时间",
    "长度": "9",
    "说 明": "本信息包产生时间。 以数字表示年（2字节）、月、日、时、分、秒、毫秒（2字节）"
  }
]
...
[
  {
    "字段": "最大车头所在逻辑区段内偏移量",
    "长度": "4",
    "说明": "单位：cm默认值：0xFFFFFFFF；有效范围：0x0~0xFFFFFFF"
  },
  {
    "字段": "最小车头所在逻辑区段编号",
    "长度": "4",
    "说明": "车尾所在位置默认值：0x00000000；无效值：0xFFFFFFF"
  },
  {
    "字段": "最小车头所在逻辑区段内偏移量",
    "长度": "4",
  ...
  },
  {
    "字段": "运行方向",
    "长度": "1",
    "说明": "ATS系统内的列车运营方向0x01：下行；0x02：上行；0x00：未知；其他无效"
  },
  {
    "字段": "车载ATP报告方向",
    "长度": "1",
    "说明": "ATP系统位置报告中的列车方向0x01：下行；0x02：上行；0x00：未知；其他无效"
  },
  {
    "字段": "车轮转向",
    "长度": "1",
    "说明": "车轮正转（前进）；0x01；车轮反转（后退）；0x02；其他值无效。车轮不转动时按正转发送"
  },
  {
    "字段": "早晚点标志",
    "长度": "1",
    "说明": "0为正点，1为早点，2为晚点，其他无效"
  },
  {
    "字段": "早晚点时间",
    "长度": "4",
  ...
  },
  {
    "字段": "列车通信状态",
    "长度": "1",
    "说明": "高1bit位1表示ATP切除，0表示ATP未切除，低7bit表示列车状态，1为通信车，2为非通信车，3为通信状态未知，其他无效。补充说明：若某一方ATS无ATP切除功能，则对方作为信息接收方可不处理本字段高1bit，本方作为信息发送方应将本字段高1bit置为0"
  }
]
...
[
  {
    "字段": "列车状态",
    "长度": "1",
    "说明": "0x01:CBTC车;0x02:非CBTC车;其他无效"
  },
  {
    "字段": "停准停稳",
    "长度": "1",
    "说明": "1:停准且停稳;2:未停准或未停稳;其他无效"
  },
  {
    "字段": "列车速度信息",
    "长度": "2",
    "说明": "数值,单位:cm/s默认值:0xFFFF"
  },
  {
    "字段": "车门关闭状态",
    "长度": "1",
    "说明": "0x00:开;0x01:关;0xFF:门旁路;其他无效。如无旁路状态应按照关门处理"
  },
  {
    "字段": "ATP系统模式",
    "长度": "1",
    "说明": "0x01:RM(限制人工驾驶模式);0x02:CM(列车自动防护模式);0x03:AM(列车自动运行模式);0x04:LUM;0xFF:默认值;其他无效"
  },
  {
    "字段": "ATO工作模式",
    "长度": "1",
    "说明": "0x00:ATO未建立;0x03:AM自动驾驶;其他无效"
  },
  {
    "字段": "区间运行调整命令",
    "长度": "2",
    "说明": "运行等级或区间运行时间(列车从前一站发车到下一站停车时间,单位:s),可根据工程需求确定"
  },
  {
    "字段": "跳停命令",
    "长度": "1",
    "说明": "0x55:有;0xAA:无;0xFF:状态无意义;其他无效"
  },
  {
    "字段": "扣车命令",
    "长度": "1",
...
[
  {
    "字段": "列车阻塞 $ ^{*} $",
    "长度": "1",
    "说明": "0x01:列车阻塞;0x02:列车非阻塞;其他无效"
  },
  {
    "字段": "列车激活端",
    "长度": "1",
    "说明": "同《城市轨道交通 基于通信的列车运行控制系统(CBTC)互联互通接口规范 第2部分:CBTC系统车地连续通信协议》中的定义"
  },
  {
    "字段": "列车完整性",
    "长度": "1",
    "说明": "0x01:完整;0x02:不完整;其他无效"
  },
  {
    "字段": "列车紧急制动状态",
    "长度": "1",
    "说明": "0x01:紧急制动;0x02:无紧急制动;其他无效"
  },
  {
    "字段": "......"
  },
  ...
  {
    "字段": "“列车阻塞”表示列车由于停车或低速行驶,处在某区间内时间是否超出设定时限(可设置)的状态信息。",
    "长度": "“列车阻塞”表示列车由于停车或低速行驶,处在某区间内时间是否超出设定时限(可设置)的状态信息。",
    "说明": "“列车阻塞”表示列车由于停车或低速行驶,处在某区间内时间是否超出设定时限(可设置)的状态信息。"
  }
]
...
[
  {
    "字段": "预计交出时间",
    "长度": "7",
    "说明": "计划调整后的预计交出时间。以数字表示年(2字节)、月、日、时、分、秒,7个字节全为0表示列车已经实际交出。无计划信息时取值为0xFFFFFFFFFFFF"
  },
  {
    "字段": "预计交出时刻的计划偏离时分",
    "长度": "4",
    "说明": "表示计划在本线路调整完成后剩余的时间偏差值,也即需要下一线路ATS进行计划调整的时间偏差值,无计划信息列车按照准点处理。bit0~bit29为偏离时分数值,单位:s;bit30~bit31为早晚点标志,0为准点,1为早点,2为晚点"
  },
  {
    "字段": "实际交出时间",
    "长度": "7",
    "说明": "列车实际运行的交出时间。以数字表示年(2字节)、月、日、时、分、秒,7个字节全为0表示列车还未实际交出"
  },
  {
    "字段": "实际交出时刻的计划偏离时分",
    "长度": "4",
    "说明": "早晚点信息。无计划信息列车按照准点处理。bit0~bit29为偏离时分数值,单位:s;bit30~bit31为早晚点标志,0为准点,1为早点,2为晚点"
  },
  {
    "字段": "跨线区间的预计运行时间",
    "长度": "2",
...
[
  {
    "字段": "操作ID",
    "长度": "16",
    "说明": "UUID,命令唯一标识,每次人工/自动操作均有不同的ID"
  },
  {
    "字段": "接入站所在线路号",
    "长度": "4",
  ...
  },
  {
    "字段": "时间",
    "长度": "9",
    "说明": "本信息包产生时间以数字表示年(2字节)、月、日、时、分、秒、毫秒(2字节)"
  },
  {
    "字段": "站台数量",
    "长度": "2",
  ...
  },
  {
    "字段": "跳停类型",
    "长度": "2",
    "说明": "数字,1为无跳停,2为所有列车跳停,3为有指定列车跳停,0xFFFF为默认值,其他无效"
  },
  {
    "字段": "列车数量",
    "长度": "2",
    "说明": "数字,如跳停类型为1无跳停和2所有列车跳停,本字段应为0,跳停列车信息仅在跳停类型为指定列车跳停时有效"
  },
  {
    "字段": "源线路号",
    "长度": "4",
...
[
  ...
  },
  {
    "字段": "车组所属线路号",
    "长度": "4",
    "说明": "详见附录A.3,车组号长度为0时无效"
  },
  {
    "字段": "车组号长度",
    "长度": "1",
    "说明": "数字,车组号长度为0时,表示指定车次跳停,表号、车次号、车组号不能同时为空"
  },
  {
    "字段": "车组号",
    "说明": "字符,个数由车组号长度指定,详见附录A.3"
  ...
  {
    "字段": "下一停车站台编号",
    "说明": "前方最近的停车站台编号,定义同站台编号,默认值为0xFFFF"
  },
  {
    "字段": "......"
  },
...
[
  ...
  },
  {
    "字段": "回执单位编码",
    "长度": "2",
    "说明": "单位类型为中心时,单位编码为调度台ID,服务器ID为0单位类型为车站时,单位编码为站码"
  },
  {
    "字段": "回执单位名称长度",
    "长度": "1",
  ...
  },
  {
    "字段": "回执发送时间",
    "长度": "9",
    "说明": "命令回执发送时的系统时间,以数字表示年、月、日、时、分、秒、毫秒(2字节)"
  },
  {
    "字段": "接收结果状态",
    "长度": "1",
    "说明": "数值,0x01:接收成功,0x02:接收失败接收失败时,接收返回信息字段为失败原因,接收成功时,接收返回信息长度为0其他无效"
  },
  {
    "字段": "接收返回信息长度",
    "长度": "2",
...
[
  {
    "设备类型": "0x11",
    "设备类型名称": "逻辑区段",
    "备注": "CHINA ASSOCIATION OF METROS"
  },
  {
    "设备类型": "0x16",
    "设备类型名称": "物理区段"
  ...
  },
  {
    "设备类型": "0x41",
    "设备类型名称": "表示灯",
    "备注": "例如，G1通道1、接口电源灯等表示灯，自动进路、自动触发表示灯"
  },
  {
    "设备类型": "0x42",
    "设备类型名称": "联锁计数器",
    "备注": "“联锁计数器”是指ATS界面上用于显示联锁倒计时（延时解锁等情况下）的设备"
  },
  {
    "设备类型": "0x52",
    "设备类型名称": "站台紧急关闭"
...
[
  ...
  },
  {
    "比特位": "10",
    "状态": "计轴复位",
    "值": "0:无计轴复位(或默认值)1:有计轴复位",
    "说明": "若ATS没有此信息,应发送默认值,作为信息接收方时可不处理本字段"
  },
  {
    "比特位": "11",
    "状态": "预留"
...
[
  {
    "比特位": "24,25",
    "状态": "OD占用",
    "值": "00:默认 01:非法 10:出清 11:占用",
    "说明": "若ATS没有此信息,应 发送默认值,作为信息接 收方时可不处理本字段"
  },
  {
    "比特位": "26~31",
    "状态": "预留"
...
[
  ...
  },
  {
    "比特位": "31,32",
    "状态": "OD 占用",
    "值": "00:默认01:非法10:出清11:占用",
    "说明": "若 ATS 没有此信息, 应发送默认值, 作为信息接收方时可不处理本字段"
  },
  {
    "比特位": "33~39",
    "状态": "预留"
...
[
  {
    "比特位": "0",
    "状态": "信息标志位",
    "值": "0:状态信息无效临时限速状态为未知1:状态信息有效",
    "说明": "表示ATS间此临时限速区段信息是否有效"
  },
  {
    "比特位": "1,2",
    "状态": "CI限速状态",
    "值": "00:无限速01:限速验证通过10:限速执行成功11:取消验证通过",
    "说明": "若ATS没有限速验证通过、取消验证通过状态，作为信息接收方时可不处理本字段的上述状态"
  },
  {
    "比特位": "3,4",
    "状态": "ATP限速状态",
...
[
  ...
  },
  {
    "比特位": "1~4",
    "状态": "低位颜色",
    "值": "0x00:无色0x01:绿0x02:黄0x03:红0x04:白0x05:蓝0x06:灰0x07:紫0x08:黑0x09:橙0x0a:青0x0b~0x0f:预留"
  },
  {
    "比特位": "5",
    "状态": "低位闪烁",
...
[
  {
    "比特位": "14",
    "状态": "屏蔽一次临时限速",
    "值": "0: 无屏蔽一次临时限速1: 屏蔽一次临时限速",
    "说明": "点式、联锁等级下进路内存在有临时限速的区段时，进路始端信号机不开放，屏蔽一次临时限速命令可开放始端信号机。一次有效。本项表示信号机有或无屏蔽一次临时限速的状态，默认值为0"
  },
  {
    "比特位": "15",
    "状态": "预留"
...
[
  ...
  },
  {
    "比特位": "1,2",
    "状态": "门状态",
    "值": "00:正常关门 01:正常开门 10:旁路关门 11:旁路开门",
    "说明": "若ATS没有旁路关门和旁路开门状态,作为信息接收方时以上两种状态宜按照正常关门处理"
  },
  {
    "比特位": "3~7",
    "状态": "预留",
...
[
  {
    "内容": "源线路号",
    "说明": "列车始发所在线路号，可由字母或数字组成，非跨线列车源线路号与日的线路号相同"
  },
  {
    "内容": "目的线路号",
    "说明": "列车终到所在线路号，可由字母或数字组成，非跨线列车目的线路号与源线路号相同"
  },
  {
    "内容": "列车类型",
    "说明": "1为计划车；2为头码车；3为人工车；其他无效"
  ...
  {
    "内容": "表号",
    "说明": "由两位数字或三位数字组成，默认值为“00”或“000”"
  },
  {
    "内容": "车次号",
    "说明": "由四位数字组成，默认值为“0000”，有效范围0001~9999。9000~9999作为预留车次范围（车次号范围定义可按照业主需求进行配置）"
  },
  {
    "内容": "车组所属线路号",
    "说明": "车组所属线路号，可由字母或数字组成"
  },
  {
    "内容": "车组号",
    "说明": "由三位数字组成，有效范围001~999，车组所属线路号+车组号应在互联互通线路内唯一"
  },
  {
    "内容": "目的地号",
    "说明": "可由字母或数字组成，目的线路号+目的地号在互联互通线路内应统一定义"
  },
  {
    "内容": "运行方向",
    "说明": "ATS系统内的列车运营方向0x01：下行；0x02：上行；0x00：未知；其他无效"
