...
class CompetitionRunner:
    ...
    def build_or_load_vectorstore(self, rebuild: bool = False):
        """
        构建或加载向量数据库
        使用增强的表格处理功能，更大的token限制以容纳完整表格
        """
        print("=" * 60,"构建或加载向量数据库")
        if rebuild or not os.path.exists(f"{self.storage_path}/vectors.json"):
            print("📚 构建向量数据库（使用增强的表格处理）...")
            
            # 备份旧数据库
            if rebuild and os.path.exists(f"{self.storage_path}/vectors.json"):
                import shutil
                backup_path = f"{self.storage_path}_backup"
                if os.path.exists(backup_path):
                    shutil.rmtree(backup_path)
                print(f"   📦 备份旧数据库到 {backup_path}")
                shutil.copytree(self.storage_path, backup_path)
            
            # 使用适中的token限制以容纳完整表格，同时确保不超过8000限制
            # max_token_len=600 可以包含大部分表格，cover_content=80 保证上下文连贯
            # 注意：加上元数据后总token应远小于8000
            docs = self.reader.get_content(max_token_len=600, cover_content=80)
            print(f"   总共分块数: {len(docs)}")
            
            # 统计表格chunk
            table_chunks = sum(1 for doc in docs if '【表头】' in doc or '【表格数据】' in doc)
            print(f"   包含表格的chunk: {table_chunks}")
    ...
    def process_single_question(self, query: str, question_type: str = "") -> Dict:
        ...
        print(f"\n答案: {output['answer']}")
        print(f"检索到的文档片段数: {len(output['retrieved_contexts'])}")
...
def main():
    ...
    parser.add_argument('--query', type=str, default='',
                       help='单个问题模式下的查询')
    parser.add_argument('--data-path', type=str, default='/home/honglianglu/ssd/casco/project/output_paddle/',
                       help='数据目录路径')
