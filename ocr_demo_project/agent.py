...
class RAGAgent:
    ...
    def extract_technical_terms(self, query: str) -> List[str]:
        ...
        # 提取标准号模式（如GB/T 12345-2023），优先提取完整标准号
        standard_patterns = re.findall(r'[A-Z]{2,}[\/∕\s]*[A-Z]*\s*\d+[\.\-—]\d+(?:[\-—]\d+)?', query)
        terms.extend(standard_patterns)
        
        # 提取大写缩写（如CBTC, ERTMS, IEEE等），但排除已经在标准号中的
        abbreviations = re.findall(r'\b[A-Z]{3,}[\w\-\.]*\b', query)  # 至少3个大写字母，避免 GB 这种短词
        for abbr in abbreviations:
            if not any(abbr in std for std in standard_patterns):
                terms.append(abbr)
    ...
    def multi_query_retrieve(self, query: str, k: int = 5) -> List[Dict]:
        ...
        for q in queries:
            # 使用新的query_with_source方法
            results = self.vector_store.query_with_source(q, self.embedding, k=k)
            for result in results:
                ...
                if content not in seen_contents:
                    ...
                    all_results.append({
                        'content': content,
                        'source': result.get('source'),  # 添加来源信息
                        'query': q,
                        'relevance': result.get('similarity', 1.0)
                    })
    ...
    def rerank_results(self, query: str, results: List[Dict]) -> List[Dict]:
        ...
        for result in results:
            ...
            # 计算关键词匹配度（优先匹配重要关键词）
            important_match = sum(1 for keyword in important_keywords if keyword in content_lower)
            all_match = sum(1 for keyword in query_keywords if keyword in content_lower)
            
            # 综合评分：原始相似度 * 0.7 + 关键词匹配 * 0.3
            original_similarity = result.get('similarity', 0)
            keyword_score = (important_match * 2 + all_match) / (len(important_keywords) * 2 + len(query_keywords)) if query_keywords else 0
            
            result['relevance'] = original_similarity * 0.7 + keyword_score * 0.3
...
请基于上述文档内容进行深入分析并回答。"""
    ...
    def format_output(self, result: Dict) -> Dict:
        ...
        # 格式化召回结果为字符串数组
        retrieved_contexts = []
        for r in result['results'][:10]:  # 最多返回10个结果
            # 获取文档来源
            source = r.get('source', '未知来源')
            
            # 格式化content，添加 [来源: 文件名] 前缀
            if source:
                formatted_content = f"[来源: {source}]\n{r['content']}"
            else:
                formatted_content = r['content']
            
            retrieved_contexts.append(formatted_content)
