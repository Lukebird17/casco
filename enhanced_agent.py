...
# import os
# from LLM import OpenAIChat
# from cot.auto_cot_prompting import AutoCotPromptBuilder
...
#     def query_with_full_features(self, query: str, max_retries: int = 2, force_query_type: Optional[str] = None) -> Dict:
#         """
#         完整功能的查询（集成所有增强特性）
#         Args:
#             query: 用户问题
...
#         # 分析问题类型
#         if force_query_type in ['basic', 'intermediate', 'advanced']:
...
class EnhancedRAGAgent:
    ...
    def __init__(self, vector_store: Union[VectorStore, List[VectorStore]], llm: OpenAIChat, 
                embedding: OpenAIEmbedding, enable_tracking: bool = True):
    ...
    def analyze_query_type(self, query: str) -> str:
        ...
        Returns:
            问题类型
        """
        advanced_keywords = ['对比', '演变', '如何','怎么','计算','区别','哪','版本', '分别', '不同', 'baseline', '变化', '差异']
        # 中级题特征：排名、列出、完整、所有
        intermediate_keywords = ['排第几', '排名', '列出', '完整', '所有', '分别是']
        query_lower = query.lower()
    ...
    def multi_query_retrieve(self, query: str, k: int = 5) -> List[Dict[str, Union[str, float]]]:
        ...
        for store in self.vector_stores:
            results = store.query(
                ...
                EmbeddingModel=self.embedding,
                k=k,
                llm_translator_class=OpenAIChat,
        ...
        for q in queries:
            ...
            for store in self.vector_stores:
                ...
                results_with_metadata.extend(results)
